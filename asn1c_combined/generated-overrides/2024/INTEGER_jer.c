...
INTEGER__jer_body_decode(const asn_TYPE_descriptor_t *td, void *sptr,
    ...
    case ST_DIGITS_TRAILSPACE:
        ...
        case ASN_STRTOX_OK:
            // EDIT: use intmax_t instead of long for Windows compatibility
            if(specs && specs->field_unsigned && (uintmax_t) dec_value <= UINTMAX_MAX) {
                break;
            } else if(dec_value >= INTMAX_MIN && dec_value <= INTMAX_MAX) {
                break;
            } else {
                ASN_DEBUG("INTEGER exceeds intmax_t range");
            }
            /* Fall through */
